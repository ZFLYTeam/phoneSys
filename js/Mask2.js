function openDiv2(newDivID2) {
    ...
    newMask.style.zIndex = "8888"; //遮罩层zIndex
    ...
    //新弹出层
    var newDiv2 = document.getElementById("newDiv2");   //从文档流中获取操作对象
    newDiv2.style.display = "block";   //显示为块级元素
    newDiv2.style.zIndex = "9999";   //索引为9999
    newDiv2.style.position = "fixed";   //固定定位，使该层滚动条滚动时垂直居中

    //关闭遮罩层和弹出层
    var myClose = document.getElementById("close2");   //从文档流中获取关闭对象元素
    myClose.onclick = function () {   //当点击关闭对象元素时执行该方法
        newMask.style.display = "none";   //隐藏遮罩层
        newDiv2.style.display = "none";   //隐藏弹出层
    }
}